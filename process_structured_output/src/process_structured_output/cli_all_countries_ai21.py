"""CLI for batch processing all countries assigned to AI21."""
...
def main() -> int:
    """
    CLI entry point for processing all AI21-assigned countries.

    Usage:
        python -m process_structured_output.cli_all_countries_ai21
        all-countries-ai21
        all-countries-ai21 --skip-cities

    Returns:
        0 on success (all countries processed), 1 on any failure
    """
    parser = argparse.ArgumentParser(
        description="Process all countries assigned to AI21",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  all-countries-ai21
  all-countries-ai21 --skip-cities
  all-countries-ai21 --dry-run
        """,
    )
    parser.add_argument(
        "--skip-cities",
    ...
    # Get list of AI21 countries from utilities
    try:
        countries = get_countries_by_llm("AI21")
    except (ImportError, ValueError) as e:
        print(f"[X] Error loading countries list: {e}")
        return 1

    print(f"\n=== Processing {len(countries)} AI21 Countries ===\n")
