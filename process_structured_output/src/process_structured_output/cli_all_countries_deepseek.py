"""CLI for batch processing all countries assigned to DeepSeek."""
...
def main() -> int:
    """
    CLI entry point for processing all DeepSeek-assigned countries.

    Usage:
        python -m process_structured_output.cli_all_countries_deepseek
        all-countries-deepseek
        all-countries-deepseek --skip-cities

    Returns:
        0 on success (all countries processed), 1 on any failure
    """
    parser = argparse.ArgumentParser(
        description="Process all countries assigned to DeepSeek",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  all-countries-deepseek
  all-countries-deepseek --skip-cities
  all-countries-deepseek --dry-run
        """,
    )
    parser.add_argument(
        "--skip-cities",
    ...
    # Get list of DeepSeek countries from utilities
    try:
        countries = get_countries_by_llm("DeepSeek")
    except (ImportError, ValueError) as e:
        print(f"[X] Error loading countries list: {e}")
        return 1

    print(f"\n=== Processing {len(countries)} DeepSeek Countries ===\n")
