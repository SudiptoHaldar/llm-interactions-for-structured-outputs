"""CLI for batch processing all countries assigned to Anthropic."""
...
def main() -> int:
    """
    CLI entry point for processing all Anthropic-assigned countries.

    Usage:
        python -m process_structured_output.cli_all_countries_anthropic
        all-countries-anthropic
        all-countries-anthropic --skip-cities

    Returns:
        0 on success (all countries processed), 1 on any failure
    """
    parser = argparse.ArgumentParser(
        description="Process all countries assigned to Anthropic",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  all-countries-anthropic
  all-countries-anthropic --skip-cities
  all-countries-anthropic --dry-run
        """,
    )
    parser.add_argument(
        "--skip-cities",
    ...
    # Get list of Anthropic countries from utilities
    try:
        countries = get_countries_by_llm("Anthropic")
    except (ImportError, ValueError) as e:
        print(f"[X] Error loading countries list: {e}")
        return 1

    print(f"\n=== Processing {len(countries)} Anthropic Countries ===\n")
