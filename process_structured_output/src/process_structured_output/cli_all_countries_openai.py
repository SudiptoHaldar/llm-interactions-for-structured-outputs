"""CLI for batch processing all countries assigned to OpenAI."""
...
def main() -> int:
    """
    CLI entry point for processing all OpenAI-assigned countries.

    Usage:
        python -m process_structured_output.cli_all_countries_openai
        all-countries-openai
        all-countries-openai --skip-cities

    Returns:
        0 on success (all countries processed), 1 on any failure
    """
    parser = argparse.ArgumentParser(
        description="Process all countries assigned to OpenAI",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  all-countries-openai
  all-countries-openai --skip-cities
  all-countries-openai --dry-run
        """,
    )
    parser.add_argument(
        "--skip-cities",
    ...
    # Get list of OpenAI countries from utilities
    try:
        countries = get_countries_by_llm("OpenAI")
    except (ImportError, ValueError) as e:
        print(f"[X] Error loading countries list: {e}")
        return 1

    print(f"\n=== Processing {len(countries)} OpenAI Countries ===\n")
