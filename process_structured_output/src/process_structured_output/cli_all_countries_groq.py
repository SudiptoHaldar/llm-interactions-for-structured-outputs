"""CLI for batch processing all countries assigned to Groq."""
...
def main() -> int:
    """
    CLI entry point for processing all Groq-assigned countries.

    Usage:
        python -m process_structured_output.cli_all_countries_groq
        all-countries-groq
        all-countries-groq --skip-cities

    Returns:
        0 on success (all countries processed), 1 on any failure
    """
    parser = argparse.ArgumentParser(
        description="Process all countries assigned to Groq",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  all-countries-groq
  all-countries-groq --skip-cities
  all-countries-groq --dry-run
        """,
    )
    parser.add_argument(
        "--skip-cities",
    ...
    # Get list of Groq countries from utilities
    try:
        countries = get_countries_by_llm("Groq")
    except (ImportError, ValueError) as e:
        print(f"[X] Error loading countries list: {e}")
        return 1

    print(f"\n=== Processing {len(countries)} Groq Countries ===\n")
