"""CLI for batch processing all countries assigned to Google."""
...
def main() -> int:
    """
    CLI entry point for processing all Google-assigned countries.

    Usage:
        python -m process_structured_output.cli_all_countries_google
        all-countries-google
        all-countries-google --skip-cities

    Returns:
        0 on success (all countries processed), 1 on any failure
    """
    parser = argparse.ArgumentParser(
        description="Process all countries assigned to Google",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  all-countries-google
  all-countries-google --skip-cities
  all-countries-google --dry-run
        """,
    )
    parser.add_argument(
        "--skip-cities",
    ...
    # Get list of Google countries from utilities
    try:
        countries = get_countries_by_llm("Google")
    except (ImportError, ValueError) as e:
        print(f"[X] Error loading countries list: {e}")
        return 1

    print(f"\n=== Processing {len(countries)} Google Countries ===\n")
